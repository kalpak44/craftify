...
/**
 * BOMCreationPage — self-contained React + Tailwind (pure JS, no TS types)
 * - Mock Items master
 * - General form (parent item, BOM ID, revision, status, effective date, description)
 * - Components table (add/clone/remove rows, inline picker, qty/uom/notes)
...
function BOMCreationPage() {
    ...
    }, [parentItemId, bomId, revision, status, effectiveDate, description, rows]);
    ...
    const handleSaveDraft = () => {
        setLastSavedAt(new Date());
        setDirty(false);
        alert("Draft saved (mock). No backend calls yet.");
    };

    const handleActivate = () => {
        if (errors.length) {
            alert("Please resolve errors before activation.");
            return;
        }
        alert("BOM activated (mock).Status would switch to Active.");
        setStatus("Active");
    };
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {errors.length > 0 && (
                        <div className="rounded-xl border border-red-500/40 bg-red-500/10 p-4 text-sm">
                            <div className="font-semibold text-red-300 mb-1">Please fix the following before
                                activation:
                            </div>
                ...
                <aside className="lg:sticky lg:top-6 space-y-4">
                    <div className="rounded-2xl border border-white/10 bg-gray-900/60 p-4">
                        ...
                        <div className="mt-3 grid grid-cols-2 gap-3 text-sm">
                            ...
                            <div className="rounded-xl bg-gray-800/60 p-3 border border-white/10">
                                ...
                                <div className="text-lg font-semibold text-gray-100">€{rolledCost.toFixed(2)}</div>
                            </div>
                    ...
                    <div className="rounded-2xl border border-white/10 bg-gray-900/60 p-4 text-xs text-gray-400">
                        ...
                        <ul className="list-disc pl-5 space-y-1">
                            ...
                            <li>Keep sub-assemblies out for now; add later via multi-level BOMs.</li>
                        </ul>
                    </div>
                </aside>
            </section>
