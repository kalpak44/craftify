...
/** Dummy implementation of ItemsBatchDeleteApi. Returns the number of IDs received as "deleted". */
@RestController
public class ItemsBatchDeleteApiController implements ItemsBatchDeleteApi {
  ...
  public ResponseEntity<ItemsBatchDeletePost200Response> itemsBatchDeletePost(
    ...
    int deleted =
        (ids == null) ? 0 : (int) ids.stream().filter(Objects::nonNull).distinct().count();
