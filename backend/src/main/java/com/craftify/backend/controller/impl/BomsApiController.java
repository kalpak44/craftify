...
public class BomsApiController implements BomsApi {
  ...
  public ResponseEntity<BomPage> bomsGet(
                  ...
                  b ->
                      (b.getId() != null && b.getId().toLowerCase(Locale.ROOT).contains(qq))
                          || (b.getProductName() != null
                              && b.getProductName().toLowerCase(Locale.ROOT).contains(qq)))
              .toList();
    }
    if (status != null) {
      BomStatus st = status;
    ...
    if ("id".equalsIgnoreCase(sortKey)) {
      cmp =
          Comparator.comparing(BomDetail::getId, Comparator.nullsLast(String::compareToIgnoreCase));
    } else if ("productId".equalsIgnoreCase(sortKey)) {
      cmp =
          Comparator.comparing(
              BomDetail::getProductId, Comparator.nullsLast(String::compareToIgnoreCase));
    } else {
      cmp =
          Comparator.comparing(
              BomDetail::getProductName, Comparator.nullsLast(String::compareToIgnoreCase));
    }
  ...
  public ResponseEntity<BomDetail> bomsPost(BomDetail req) {
    ...
    BomDetail created =
        new BomDetail()
            ...
            .note(req.getNote())
            .components(
                req.getComponents() != null
                    ? new ArrayList<>(req.getComponents())
                    : new ArrayList<>())
  ...
  public ResponseEntity<BomDetail> bomsIdPut(String id, String ifMatch, BomDetail req) {
    ...
    existing.setComponents(
        req.getComponents() != null ? new ArrayList<>(req.getComponents()) : new ArrayList<>());
