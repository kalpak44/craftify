...
/**
 * Dummy implementation of ItemsImportApi. Replace with real CSV parsing + persistence logic later.
 */
@RestController
public class ItemsImportApiController implements ItemsImportApi {
  ...
  public ResponseEntity<ImportResult> itemsImportPost(MultipartFile file, String mode) {
    log.info(
        "POST /items:import called (dummy implementation) mode={} fileName={} size={}",
        mode,
        file.getOriginalFilename(),
        file.getSize());

    // Build a dummy result
    ImportResult result = new ImportResult().created(3).updated(1).errors(List.of());
