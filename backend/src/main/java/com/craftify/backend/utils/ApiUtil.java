...
/**
 * Utility used by generated API interfaces to write example responses. Called from default methods
 * when no controller implementation exists.
 */
public final class ApiUtil {
  ...
  public static void setExampleResponse(
      NativeWebRequest request, String mediaType, String exampleBody) {
    Optional.ofNullable(request.getNativeResponse(HttpServletResponse.class))
        .ifPresent(
            response -> {
              try {
                ...
                log.trace(
                    "Example response written with mediaType={} length={}",
                    mediaType,
                    exampleBody.length());
